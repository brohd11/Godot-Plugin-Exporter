...
static func set_popup_position(popup:PopupMenu, offset:=Vector2i.ZERO):
	offset = offset * _EIBackport.get_ins().ei.get_editor_scale()
...
static func _get_icon(icon_name:String, theme_type:String="EditorIcons"):
	return _EIBackport.get_ins().ei.get_base_control().get_theme_icon(icon_name, theme_type)
...
const _BPSV_PATH_popup_menu_path_helper = "res://addons/plugin_exporter_backport/src/class/remote/addons/addon_lib/brohd/alib_runtime/popup_menu/popup_menu_path_helper.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
