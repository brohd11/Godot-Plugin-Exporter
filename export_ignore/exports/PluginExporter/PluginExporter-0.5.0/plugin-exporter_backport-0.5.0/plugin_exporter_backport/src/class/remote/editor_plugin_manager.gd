...
func add_context_menu_plugins(context_menu_plugins=null) -> void:
	...
	for path_or_script in context_menu_plugins:
		...
		if "SLOT" in script:
			var ins = script.new()
			if ins is EditorContextMenuPluginCompat:
...
func add_syntax_highlighters(highlighters=null) -> void:
	...
	for path_or_script in highlighters:
		...
		_EIBackport.get_ins().ei.get_script_editor().register_syntax_highlighter(highlighter)
...
func _remove_syntax_highlighters() -> void:
	for key in _syntax_highlighter_instances.keys():
		...
		_EIBackport.get_ins().ei.get_script_editor().unregister_syntax_highlighter(highlighter)
...
const _BPSV_PATH_editor_plugin_manager = "res://addons/plugin_exporter_backport/src/class/remote/editor_plugin_manager.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
