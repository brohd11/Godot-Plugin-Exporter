...
#static var preload_regex:RegEx<- Backport Static Var
const UtilsLocal = preload("res://addons/plugin_exporter_backport/src/class/utils_local.gd")
const UtilsRemote = preload("res://addons/plugin_exporter_backport/src/class/utils_remote.gd") #>remote
...
func _init() -> void:
	set_preload_regex(UtilsRemote.URegex.get_preload_path())
...
static func get_preload_path(line):
	if get_preload_regex() == null:
		set_preload_regex(UtilsRemote.URegex.get_preload_path())
	
	if _check_for_comment(line, "preload("):
		return
	var _match = get_preload_regex().search(line)
	if _match:
...
const _BPSV_PATH_parse_base = "res://addons/plugin_exporter_backport/src/class/export/parse/parse_base.gd"

static func get_preload_regex():
	return _BackportStaticVar.get_ins().get_var(_BPSV_PATH_parse_base, 'preload_regex', null)
static func set_preload_regex(value):
	return _BackportStaticVar.get_ins().set_var(_BPSV_PATH_parse_base, 'preload_regex', value)
### PLUGIN EXPORTER STATIC VAR BACKPORT
