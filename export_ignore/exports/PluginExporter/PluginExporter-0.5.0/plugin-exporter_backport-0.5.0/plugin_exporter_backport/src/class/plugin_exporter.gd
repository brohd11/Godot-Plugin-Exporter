...
const Plugin = preload("res://addons/plugin_exporter_backport/plugin.gd")
...
static func gui_open(plugin_name:String):
	...
	if not FileAccess.file_exists(export_file_path):
		...
		return
	if not is_instance_valid(Plugin.get_instance()):
		printerr("PluginExporter instance not found. Make sure plugin is enabled.")
		return
	
	var dock_manager = Plugin.get_instance().new_gui_instance()
...
const _BPSV_PATH_plugin_exporter = "res://addons/plugin_exporter_backport/src/class/plugin_exporter.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
