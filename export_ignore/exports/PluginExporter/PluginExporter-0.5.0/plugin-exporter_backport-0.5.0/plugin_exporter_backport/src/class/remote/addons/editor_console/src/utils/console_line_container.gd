...
func _ready() -> void:
	...
	console_button.icon = _EIBackport.get_ins().ei.get_base_control().get_theme_icon("Terminal", &"EditorIcons")
...
class ConsoleLineEdit extends CodeEdit:
	...
	func _build_popup(item_dict):
		...
		_EIBackport.get_ins().ei.get_base_control().add_child(popup)
		var wind_pos
		if BACKPORTED >= 3:
...
const _BPSV_PATH_console_line_container = "res://addons/plugin_exporter_backport/src/class/remote/addons/editor_console/src/utils/console_line_container.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
