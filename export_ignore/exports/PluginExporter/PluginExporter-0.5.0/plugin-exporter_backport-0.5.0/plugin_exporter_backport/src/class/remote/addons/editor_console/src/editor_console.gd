...
func _init() -> void:
	...
	_accent_color = _EIBackport.get_ins().ei.get_base_control().get_theme_color("accent_color", &"Editor").to_html()
...
func _add_console_line_edit():
	...
	if not filter_check is LineEdit:
...
const _BPSV_PATH_editor_console = "res://addons/plugin_exporter_backport/src/class/remote/addons/editor_console/src/editor_console.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
