...
static func get_completion(raw_text:String, commands:Array, args:Array, editor_console) -> Dictionary:
	...
	if commands.size() > 1:
		...
		if c_2 == CALL_COMMAND and raw_text.find(" --") > -1:
			var script = _EIBackport.get_ins().ei.get_script_editor().get_current_script()
...
static func list_args(script:Script, args:Array):
	...
	if args_array.is_empty():
		...
		return
	var class_name_color = _EIBackport.get_ins().ei.get_editor_settings().get_setting(_ARG_CLASS_COLOR_SETTING).to_html()
	var args_strings = []
	for dict in args_array:
		var name = dict.get("name")
		var type = dict.get("type")
		var arg_str = "%s:[color=%s]%s[/color]" % [name, class_name_color, MiscBackport.type_string_compat(type)]
...
const UClassDetail = preload("res://addons/plugin_exporter_backport/src/class/remote/addons/addon_lib/brohd/alib_editor/utils/src/u_class_detail.gd")
### Plugin Exporter Global Classes

### PLUGIN EXPORTER STATIC VAR BACKPORT
const _BPSV_PATH_console_script = "res://addons/plugin_exporter_backport/src/class/remote/addons/editor_console/src/default_commands/console_script.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
