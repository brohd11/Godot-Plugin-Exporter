...
func _init() -> void:
	...
	#var pattern = "(?:\\bextends\\b\\s+|preload\\s*\\(|load\\s*\\()[\\s\"']*\\K((?:res|uid)://[^\"'\\)]+)"
...
func get_direct_dependencies(file_path:String) -> Dictionary:
	...
	while not file_access.eof_reached():
		...
		for tok in tokens: ## Check for global classes
...
const _BPSV_PATH_parse_gd = "res://addons/plugin_exporter_backport/src/class/export/parse/parse_gd.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
