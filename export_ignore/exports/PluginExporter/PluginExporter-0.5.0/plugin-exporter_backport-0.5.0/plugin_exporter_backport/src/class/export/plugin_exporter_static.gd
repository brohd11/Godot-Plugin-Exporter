...
static func _print_result(success:bool, export_config_path:String):
	...
	var accent_color = _EIBackport.get_ins().ei.get_base_control().get_theme_color("accent_color", &"Editor").to_html()
	if success:
...
static func new_plugin(plugin_dir_name, create_export:=true):
	...
	_EIBackport.get_ins().ei.get_resource_filesystem().scan()
...
const _BPSV_PATH_plugin_exporter_static = "res://addons/plugin_exporter_backport/src/class/export/plugin_exporter_static.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
