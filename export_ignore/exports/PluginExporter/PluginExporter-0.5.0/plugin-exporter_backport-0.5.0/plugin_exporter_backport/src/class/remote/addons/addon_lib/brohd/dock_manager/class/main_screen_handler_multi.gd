...
const UName = preload("res://addons/plugin_exporter_backport/src/class/remote/addons/addon_lib/brohd/alib_runtime/utils/src/u_name.gd")
...
func _on_main_screen_bar_button_pressed(button:Button):
	...
	for plugin_button in plugin_buttons.keys():
		...
		if button != plugin_button:
			...
			_EIBackport.get_ins().ei.set_main_screen_editor.call_deferred(main_screen_button.text)
...
func _get_control_icon(panel_control):
	...
	if "icon" in plugin_base_control: # change load method to allow for editor nodes
		...
		return _EIBackport.get_ins().ei.get_base_control().get_theme_icon("Node", &"EditorIcons")
...
const _BPSV_PATH_main_screen_handler_multi = "res://addons/plugin_exporter_backport/src/class/remote/addons/addon_lib/brohd/dock_manager/class/main_screen_handler_multi.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
