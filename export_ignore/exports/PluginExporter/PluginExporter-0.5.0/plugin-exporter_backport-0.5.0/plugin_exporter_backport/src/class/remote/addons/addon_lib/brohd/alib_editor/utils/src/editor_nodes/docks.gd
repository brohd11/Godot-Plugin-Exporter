...
static func get_left_h_split():
	var split = _EIBackport.get_ins().ei.get_base_control().get_child(0).get_child(1)
	return split
...
const _BPSV_PATH_docks = "res://addons/plugin_exporter_backport/src/class/remote/addons/addon_lib/brohd/alib_editor/utils/src/editor_nodes/docks.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
