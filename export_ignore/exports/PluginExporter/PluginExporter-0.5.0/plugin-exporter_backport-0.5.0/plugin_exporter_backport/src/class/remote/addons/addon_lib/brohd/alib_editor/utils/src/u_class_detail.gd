...
static func class_get_all_members(script:GDScript=null):
	if script == null:
		script = _EIBackport.get_ins().ei.get_script_editor().get_current_script()
	if script == null:
		return []
...
const _BPSV_PATH_u_class_detail = "res://addons/plugin_exporter_backport/src/class/remote/addons/addon_lib/brohd/alib_editor/utils/src/u_class_detail.gd"
### PLUGIN EXPORTER STATIC VAR BACKPORT
