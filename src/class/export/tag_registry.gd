...
## once and any sub-parser can ask which members carry a tag. Queries only see files already
## scanned - scan_files() the export's file set before relying on get_entries().
## Import-free for the same reason as dependency_tags.gd: reaching utils_local drags the export
## pipeline in, which a headless suite cannot compile.
